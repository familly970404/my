...
//							schemaHtml += '<div class="meta-attr"><img src="../apps/com.actionsoft.apps.kms/raty/img/cancel-off.png" onclick="attrsearch.radioRemoveClick(this,event)" onmouseover="attrsearch.radioRemoveOver(this,event)" onmouseout="attrsearch.radioRemoveOut(this,event)" alt="x" class="raty-cancel" title="清除"></div>';
            ...
            attrsearch.attrsearchGridConfig = {
                ...
                colModel: [{
                    ...
                    render: function (ui) {
                        ...
                        if (getCharCodeLength(ui.rowData[ui.dataIndx]) > 46) {// 显示qtip
							if (ui.rowData['hasPerm'] != true) {
								...
								cardNameHtml += "<span title=\"" + ui.rowData[ui.dataIndx] + "\"  style='cursor:pointer;color:#0000ee; position: relative;top: -7px;' onclick='parent.browseCard.browse(\"" + ui.rowData.cardId + "\",\"\",false,\"" + ui.rowData.dimensionId + "\");parent.parent.stopPropagation(event);'>" + ui.rowData[ui.dataIndx] + "</span>";
							}
                        } else {
							if (ui.rowData['hasPerm'] != true) {
					...
					render: function (ui) {
						...
						if (ui.rowData['hasPerm'] != true) {
							btnHtml += "<span class='opt_icon borrow' onclick='fullsearch.borrowCard(\"" + ui.rowData['cardId'] + "\",\"" + ui.rowData['dimensionId'] + "\");' title='借阅'></span>";
						}
						return btnHtml;
					}
				}],
    ...
    resizeSearchGrid: function () {
		...
		attrsearch.searchGrid.height($(window).height() - $('.head-wrap').outerHeight(true) - $('.doc-type-wrap').outerHeight(true) - $('.dimension-meta-bar').outerHeight(true) - $('.filter-bar').outerHeight(true) - 2 - 30);
...
function backToTopList() {
	var iframeJQ = $('iframe[indx="search"]', parent.document);
	iframeJQ.attr('src', './w?cmd=com.actionsoft.apps.kms_knwl_search&sid=' + sid);
}